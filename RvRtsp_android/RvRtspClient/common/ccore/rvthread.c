    ...
    IN RvThread *th)
{
    ...
    RvAdThreadStart(&th->tcb, &th->id, &th->attr, th);
...
/* variable to operate on. */
    ...
    RvLockGet(&RvThreadLock,logMgr);
    for(i = 0; i < RV_THREAD_MAX_VARS; i++) {
        if(RvThreadVars[i].active == RV_FALSE) {
            RvThreadVars[i].exitfunc = exitfunc;
            RvThreadVars[i].active = RV_TRUE;
            if(name != NULL) {
                strncpy(RvThreadVars[i].name, name, RV_THREAD_MAX_VARNAMESIZE);
                RvThreadVars[i].name[RV_THREAD_MAX_VARNAMESIZE - 1] = '\0';
            } else RvThreadVars[i].name[0] = '\0';
            *index = i;
            RvLockRelease(&RvThreadLock,logMgr);
    ...
    return RvThreadErrorCode(RV_THREAD_ERROR_NOVARS);
}
    ...
    OUT RvUint32 *index)
{
    ...
    RvLockGet(&RvThreadLock,logMgr);
    for(i = 0; i < RV_THREAD_MAX_VARS; i++) {
        if(RvThreadVars[i].active && (strcmp(RvThreadVars[i].name, name) == 0)) {
            *index = i;
            RvLockRelease(&RvThreadLock,logMgr);
