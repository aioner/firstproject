                   ...
                   int tcp_bufflen, struct ares_options *options, int optmask)
{
    ...
    /* Reserve space for inprocess DNS server serving queries base on 'hosts' file */
    ...
    /* allocate an input TCP buffer for each server */
    for (i=0; i < max_servers; ++i)
    {
        status = RvMemoryAlloc(NULL, tcp_bufflen,
                               channel->logMgr, (void**)&channel->servers[i].tcp_buffer);
        if (status != RV_OK)
            return ARES_ESERVICE;
    }
    ...
    status = ares_set_options(channel, options, optmask);
    if (status == ARES_SUCCESS)
        status = init_by_defaults(channel);
    if (status != ARES_SUCCESS)
    {
        ares_destruct(channel);
        return status;
    }
...
void ares_destruct(RvDnsEngine *channel)
{
    ...
    (void)ares_cancel_queries(channel);
...
int ares_set_options(RvDnsEngine *channel, struct ares_options *options, int optmask)
{
        ...
        for (i = 0; i < options->nservers; i++)
        {
            ...
            if (status != ARES_SUCCESS)
            {
                RvLogError(channel->dnsSource, (channel->dnsSource,
                           "ares_set_options: init_server failed (status=%d)", status));
                return status;
            }
        }
    }
    ...
    if ((optmask & ARES_OPT_DOMAINS) && (options->ndomains > 0))
    {
        ...
        for (i = 0; i < options->ndomains; i++)
        {
            ...
            {
                RvLogError(channel->dnsSource, (channel->dnsSource,
                           "ares_set_options: set_search failed (status=%d)", status));
                return status;
            }
        }
    }
            ...
            if (status == ERROR_SUCCESS)
            {
                ...
                while (adaptAddr)
                {
                    ...
                    if (len)
                    {
                        ...
                        {
                            buffer[len] = 0;
                            set_search(channel,(const char*)buffer,len);
                        }
                    }
        ...
        {            
            ...
            case ERROR_OUTOFMEMORY:
                RvLogInfo(channel->dnsSource, (channel->dnsSource,
                    "RvAresConstruct (init_win2k): DnsQueryConfig notifies: there is no domain suffixes "));
                break;
            default:
                RvLogError(channel->dnsSource, (channel->dnsSource,
                    "RvAresConstruct (init_win2k): dqcFunc  (status=%d)", status));
            }
        }
        else if ((status = set_search(channel, (char*)buffer, RV_INT32_MAX)) != ARES_SUCCESS) 
        {
            RvLogError(channel->dnsSource, (channel->dnsSource,
...
void ares_configure_server_ose(RvUint32 addr, void *data)
{
	RvAddress rvaddr;
	RvDnsEngine *channel;
...
void ares_configure_search_ose(RvChar *str, RvInt maxDomains, void *data)
{
	RvDnsEngine *channel;
...
int ares_configure(RvDnsEngine *channel, int config_type)
{
    ...
    if (fp == NULL)
    {
        if (errno == ENOENT)  /* no such file */
            return ARES_SUCCESS;
        RvLogError(channel->dnsSource, (channel->dnsSource,
                   "ares_configure: Unable to open %s file", PATH_RESOLV_CONF));
        return ARES_EFILE;
    }
    ...
    if(config_type & RV_DNS_SERVERS) {
        for (dnsServerIp = dns_list[0]; ((dnsServerIp != 0) && (i < dnsNum)); i++, dnsServerIp = dns_list[i])
        {
            RvAddressIpv4Construct(&addr.data.ipv4, dnsServerIp, RV_ADDRESS_IPV4_ANYPORT);
	...
	if(RvAdAresConfigure((config_type & RV_DNS_SERVERS), (config_type & RV_DNS_SUFFIXES), ares_configure_server_ose, ares_configure_search_ose, (void *)channel) != RV_OK)
	...
	{
        RvLogError(channel->dnsSource, (channel->dnsSource,
                   "ares_configure: Configuration failed"));
		return status;
	}
...
static int set_options(RvDnsEngine *channel, const char *str)
{
    ...
    while (*p)
    {
        ...
        if (val && RvInt64IsEqual(channel->timeout, RvInt64Const(-1,0,1)))
            channel->timeout = RvInt64FromRvInt(atoi(val));
...
static int init_server(RvDnsEngine *channel, RvAddress *addr)
{
    ...
    {
        if (RvAddressCompare(&channel->servers[i].addr, addr, RV_ADDRESS_BASEADDRESS) == RV_TRUE)
            return ARES_SUCCESS;
    }
