...
#include <arpa/nameser.h>
    ...
    (RV_OS_TYPE == RV_OS_TYPE_MOPI)  || (RV_OS_TYPE == RV_OS_TYPE_PSOS)    ||  RV_ANDROID || \
...
int ares_cancel_query(RvDnsEngine *channel, unsigned int qid);

/* Cancels all active queries without actually destroying query list
 * Returns the pointer to this list
 */
rvQuery*
ares_cancel_queries(RvDnsEngine *channel);
