						...
						IN	RtspTransport	*pThis)
{
    ...
    //modified by lichao,20140623 简单修正rtsp报文不正确时导致死循环的问题
    RvSize_t before_buf_read_size = 0;
    RvUint8 parse_try_cnt = 0;
    RtspTransportRxState before_rx_state = RTSP_TRANSPORT_RX_STATE_FIRST_LINE;
    RvBool before_skip_eof = RV_FALSE;
	...
	{
        //modified by lichao,20140623 简单修正rtsp报文不正确时导致死循环的问题
        before_buf_read_size = (RvSize_t)pThis->rxBufferRead;
        before_rx_state = pThis->rxState;
        before_skip_eof = pThis->rxSkipEol;
        ...
        //报文解析没有解析
        if (before_buf_read_size >= (RvSize_t)pThis->rxBufferRead)
        {
            //状态未更新
            /*if ((pThis->rxState == before_rx_state) && (pThis->rxSkipEol == before_skip_eof))
            {
                //报文异常
                RvLogError(pThis->pLogSrc, (pThis->pLogSrc, "RtspTransportDoReceive - recv bad rtsp msg\r\n"));
                break;
            }*/
