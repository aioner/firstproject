                        ...
                        OUT HRPOOLELEM                 *pHContentLength)
{
    ...
    RvChar      strContentLength[sizeof(RvUint16)+2];	// LA 20131205 有数组溢出
                        ...
                        OUT RvRtspRangeHeader       *pHeader)
{
    ...
    str = strstr(token,"npt=");
    if(str) 
    {
        RtspHeadersNptTimeDeserialize(str + sizeof("npt=") - 1, &pHeader->startTime);

        str = strstr(str, "-");
        if (str)
        {
            RtspHeadersNptTimeDeserialize(str + sizeof("-") - 1, &pHeader->endTime);
        }
    }
    return RV_TRUE;
...
/**************************************************************************
 * RtspHeadersNptTimeDeserialize
 * ------------------------------------------------------------------------
 * General: deserializes the given RvRtspNptTime structure into the given
 *          buffer.
 *
 * Return Value: RV_TRUE    - Time range serialized
 *               RV_FALSE   - Serialization unsuccessful.
 ...
 * INOUT    :   None.
 *************************************************************************/
static RvBool RtspHeadersNptTimeDeserialize(
            IN RvChar          *buffer,
            OUT RvRtspNptTime   *time)
{
    RvUint32 hours = 0;
    RvUint32 minutes = 0;

    time->format = RV_RTSP_NPT_FORMAT_NOT_EXISTS;
    time->hours = 0;
    time->minutes = 0;
    time->seconds = .0;

    if (0 == strncmp("now", buffer, sizeof("now") - 1))
    {
        time->format = RV_RTSP_NPT_FORMAT_NOW;
        return RV_TRUE;
    }

    if (3 == sscanf(buffer, "%d:%d:%lf", &hours, &minutes, &time->seconds))
    {
        time->format = RV_RTSP_NPT_FORMAT_HHMMSS;
        time->hours = hours;
        time->minutes = minutes;
        return RV_TRUE;
    }

    if (1 == sscanf(buffer, "%lf", &time->seconds))
    {
        time->format = RV_RTSP_NPT_FORMAT_SEC;
        return RV_TRUE;
    }
