    ...
    for (;;)
    {
        ...
        if (act->iCreateWarning)
        {
            if (!rvSdpCreateWarning(pD,act))
            {
				...
				printf("%s:%d---error ---%d\n",__FUNCTION__,__LINE__,pD->iStopParseSts);
                return RV_FALSE;
            }
        }
...
parseFailed:
    retV = RV_FALSE;
    pD->iLineStatus = RvSdpStatusIgnore;//RvSdpValueParseFailed;
	printf("%s:%d---parseFailed !\n",__FUNCTION__,__LINE__);
    goto realReturn;
...
parseFail:
	//update by kongfd 2013-6-18 for hik rtsp parse
    pD->iLineStatus = RvSdpStatusIgnore;//RvSdpValueParseFailed;
    retV = RV_FALSE;
    goto realReturn;
