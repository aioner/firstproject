...
/********************************************************************************************
 * RvLogFindListenerCtxByCb - Find a listener user data (the context) set to the log manager
 * by listener callback.
 *
 * INPUT   : logMgr     - Log manager to use
 *           listener   - Listener function, called on each message
 * OUTPUT  : None
 * RETURN  : user data provided on listener registration.
 */
RVCOREAPI
...
RvBool RVCALLCONV RvLogIsSelected2(
    IN RvLogSource*     source,
    IN RvLogMessageType messageType);
#else

#define RvLogIsSelected(source, messageType) 0
/*( \
    ( source == NULL ? 0 : \
