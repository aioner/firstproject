					...
					IN	RvRtspRequest		*pRequest)
{
	...
	/*add by kongfd*/
	if (pRequest->authValid == RV_TRUE)
	{
		rpoolFree(hRPool, (HRPOOLELEM)pRequest->auth.hStr);
		pRequest->auth.hStr = NULL;
		pRequest->authValid = RV_FALSE;
					...
					OUT HRPOOLELEM			*phMessageElement)
{
	...
	/**add by kongfd */
	if (pMessage->authValid == RV_TRUE)
	{
		RtspHeadersAuthSerialize(hRPool, &pMessage->auth, &hTempElement);
		RtspUtilsHPOOLELEMAppend(hRPool, *phMessageElement, hTempElement);
		rpoolFree(hRPool, hTempElement);
						...
						OUT	RvRtspRequest		*pRequest)
{
	...
	while (cont)
	{
	 ...
	 /*add by kongfd*/
        if (strstr(token,"Authorization"))
        {
            pRequest->authValid = RtspHeadersAuthorizationDeserialize(hRPool,hHeaderElement,headerOffset,&pRequest->auth);
        }	 
	 /*add end */
						...
						OUT	RvRtspResponse		*pResponse)
{
	...
	pResponse->contentTypeValid		= RV_FALSE;
	pResponse->rtpInfoValid 		= RV_FALSE;
	pResponse->wwwauthenticateValid	= RV_FALSE;
	pResponse->hBody				= NULL;
	...
	while (cont)
	{
...
#if ((RV_RTSP_USE_RTSP_MSG == RV_YES))
            pResponse->transportValid = RtspHeadersTransportWithFieldsDeserialize(((RtspMsgResponse *)pResponse->hRtspMsgMessage)->hRtsp, pResponse->hRtspMsgMessage, hRPool,hHeaderElement,headerOffset,&pResponse->transport);
#else
		...
		/*add by kongfd*/
		else if (strstr(token, "www-authenticate"))
		{
			//wwwauthenticate
			//pResponse->rtpInfoValid = RtspHeadersRtpInfoDeserialize(hRPool,hHeaderElement,headerOffset, maxRtpInfoStreams, &pResponse->rtpInfo);
			if(pResponse->wwwauthenticateValid ==RV_FALSE)
				pResponse->wwwauthenticateValid = RtspHeaderswwwauthenticateDeserialize(hRPool,hHeaderElement,headerOffset,&pResponse->wwwauthenticate);
		}		
		/*add end*/
