...
{
    RvUint32 i;
    RvInt s, total, remaining, current;
    struct ifconf ifc;
    struct ifreq *ifrp;
...
#else  /* RV_OS_VERSION <= RV_OS_VXWORKS_2_2 */
...
#define RvHost_OS_GetIpV4List(hostName,logMgr,numOfIpsAllocated,numOfIps,ipList)  \
        RvSymGetHostByName(hostName,numOfIpsAllocated,numOfIps,ipList)
...
#define RvHost_OS_GetIpV4List(hostName,logMgr,numOfIpsAllocated,numOfIps,ipList)  \
	RvHostOseGetIpV4List(hostName,logMgr,numOfIpsAllocated,numOfIps,ipList)
 ...
 *           Other values on failure
 */
RvStatus RVCALLCONV RvHostOseGetIpV4List(
											 IN      RvChar*     hostName,
											 IN      RvLogMgr*   logMgr,
											 IN      RvUint32    numOfIpsAllocated,
											 OUT     RvUint32*   numOfIps,
											 OUT     RvAddress*  ipList)
{
	...
	host = (struct hostent*) gethostbyname_r(hostName);
	...
	free_buf((union SIGNAL **)&host);  /* free the signal buffer */
    ...
    *numOfIps = 0;
    ...
    if(s != INVALID_SOCKET)
    {
        ...
        for (i=0,j=0;  (j < (RvUint32)psl->iAddressCount) && (i < numOfIpsAllocated);  ++j)
        {
			...
			{
				pAddrInet = (struct sockaddr_in6 *)psl->Address[j].lpSockaddr;
				(void)RvAddressConstructIpv6(ipList+i, pAddrInet->sin6_addr.s6_addr, RV_ADDRESS_IPV6_ANYPORT, 0);
				++i;
			}
        }
        closesocket(s);
...
#elif (RV_OS_TYPE == RV_OS_TYPE_VXWORKS) || (RV_OS_TYPE == RV_OS_TYPE_MAC)
    ...
    OUT     RvAddress*  ipList)
{
    ...
    if (retVal == 0)
    {
        for (addrList = ifp; addrList != NULL && i<numOfIpsAllocated; addrList = addrList->ifa_next)
        {
            ...
            {
                RvUint8 *s6addr = ((struct sockaddr_in6*)addrList->ifa_addr)->sin6_addr.s6_addr;
                RvAddressConstructIpv6(ipList+i, s6addr, RV_ADDRESS_IPV6_ANYPORT, 0);
                i++;
            }
        }
    }
    ...
    OUT     RvAddress*  ipList)
{
    ...
    {
        RvUint8 *s6addr = ((struct sockaddr_in6 *)&lifc.lifc_req[j].lifr_addr)->sin6_addr.s6_addr;
        RvAddressConstructIpv6(ipList+i, s6addr, RV_ADDRESS_IPV6_ANYPORT, 0);
    }
    *numOfIps = i;
    return RV_OK;
...
#define ASCII_TO_HEX(b,i) ((((b[i]  <='9') ? b[i]-'0'   : tolower(b[i])-'a'+10)<<4) + \
    ...
    for (i = 0; (i < numOfIpsAllocated) && (fgets(buff, sizeof(buff), ifInet6) != NULL);  ++i)
    {
        ...
        RvAddressConstructIpv6(ipList+i, ipv6Addr, RV_ADDRESS_IPV6_ANYPORT, 0);
    }
...
/* note that the IPv6 address are consecutively inserted to the ipList structure
   ...
   it reached in the last for loop */
    if (i < *numOfIps)
    {
        RvUint32 j;
        j = *numOfIps-i;
        rvSts = RvHost_OS_GetIpV6List(logMgr,j,&j,ipList+i);
        if (rvSts != RV_OK)
            return rvSts;
        i += j;
    }
