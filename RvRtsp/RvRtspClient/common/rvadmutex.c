...
#if (_WIN32_WINNT >= 0x0500) && (RV_OS_TYPE != RV_OS_TYPE_WINCE)
    {

        DWORD spinCount = RvDefaultMutexAttr;
        if(InitializeCriticalSectionAndSpinCount(&mu->mtx, spinCount) == 0) /* tune spincount for MP systems */
            return RV_ERROR_UNKNOWN;
    }
#else
...
#if RV_MUTEX_HAS_TRYLOCK 
RvStatus RvAdMutexTryLock(
                       IN  RvMutex*        mu,
                       IN  RvLogMgr*       logMgr)
{
    RV_UNUSED_ARG(logMgr);
...
#if (RV_MUTEX_TYPE == RV_MUTEX_WIN32_MUTEX)
    if(WaitForSingleObject(mu->mtx, 0) != WAIT_OBJECT_0)
        return RV_ERROR_TRY_AGAIN;
#elif (RV_MUTEX_TYPE == RV_MUTEX_WIN32_CRITICAL)
    {
        BOOL b = TryEnterCriticalSection(&mu->mtx);
        if(b == FALSE) {
            return RV_ERROR_TRY_AGAIN;
        }
    }
