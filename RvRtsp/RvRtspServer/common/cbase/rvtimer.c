    ...
    OUT RvTimerQueue    *tqueue)
{
    ...
    RvTimerLogEnter((&logMgr->timerSource, "RvTimerQueueConstruct(tqueue=%p)", tqueue));
    ...
    IN RvSize_t     addsize)
{
    ...
    RvTimerLogEnter((tqueue->timerSource, "RvTimerQueueAddSize(tqueue=%p)", tqueue));
    ...
    IN void         *userdata)
{
    ...
    RvTimerLogDebug((tqueue->timerSource, "RvTimerStart: event allocated(tqueue=%p; timer=%p; event=%p)", tqueue, timer, event));
...
RvStatus RvTimerCancelEx(RvTimer *timer, RvTimerSleepChannel *ch) {
    ...
    if((event->id != timer->id) || (event->state == RV_TIMER_EVENTSTATE_NOTINUSE)) {
        ...
        RvLockRelease(&tqueue->lock,tqueue->logMgr);
        RvTimerLogWarning((tqueue->timerSource, "RvTimerCancel(tqueue=%p): Timer is not valid", tqueue));
        return RV_OK;
    }
    ...
    IN RvTimerCancelBehavior    behaviour)
{
    ...
    logMgr = timer->event->tqueue->logMgr;
    s = RvTimerGetSleepChannel(&sleepChannel, logMgr);
    if(s != RV_OK) {
        return s;
    }
    ...
    IN void         *alternativeCbContext)
{
    ...
    RvTimerLogEnter((tqueue->timerSource, "RvTimerQueueService(tqueue=%p)", tqueue));
    ...
    *eventcount = 0;
    ...
    RvTimerLogLeave((tqueue->timerSource, "RvTimerQueueService(tqueue=%p)", tqueue));
