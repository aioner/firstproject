...
!IF "$(UNDER_CE)" != ""
OS_DEPENDANT_INCLUDE = \
            "$(CORE_CCONFIG_ROOT)\os\rvwince.h" \
            "$(CORE_CCONFIG_ROOT)\tool\rvemvc.h"
!ELSE
OS_DEPENDANT_INCLUDE = \
...
COMMON_FILES = \
			   ...
			   "$(CORE_CCORE_ROOT)\rvcommoncorepublic.h" \
			   "$(CORE_CCORE_ROOT)\rvccoredefs.h"  \
			   "$(CORE_CCORE_ROOT)\rvstdio.h"  \
               "$(CORE_NETDRIVERS_ROOT)\rvnettypes.h"  \
               "$(CORE_NETDRIVERS_ROOT)\rvtransport.h"  \
               "$(CORE_ROOT)\ipsec\rvimsipsecpublic.h"  \
               "$(CORE_CCONFIG_ROOT)\tool\rvmsvc.h" \
               "$(CORE_CUTILS_ROOT)\rvansi.h" \
		       $(OS_DEPENDANT_INCLUDE) \
