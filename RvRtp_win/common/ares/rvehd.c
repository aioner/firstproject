...
/* Ensure host file logic is not compiled on systems,
where the host file neither is defined by user nor exists */

#if defined(RV_DNS_USES_HOSTS) && RV_DNS_USES_HOSTS

#  if (RV_OS_TYPE != RV_OS_TYPE_WIN32) && (RV_OS_TYPE != RV_OS_TYPE_LINUX) && \
    (RV_OS_TYPE != RV_OS_TYPE_SOLARIS) && (RV_OS_TYPE != RV_OS_TYPE_FREEBSD) && \
    (RV_OS_TYPE != RV_OS_TYPE_NETBSD) && (RV_OS_TYPE != RV_OS_TYPE_MAC) && (RV_OS_TYPE != RV_OS_TYPE_IPHONE)

#    undef RV_DNS_USES_HOSTS
#    define RV_DNS_USES_HOSTS 0

#    if defined(_MSC_VER)
#      pragma message("Warning: Using hosts file for DNS resolution isn't supported on this OS, setting RV_DNS_USES_HOSTS to 0")
#    elif defined(__GNUC__)
#      warning "Using hosts file for DNS resolution isn't supported on this OS, setting RV_DNS_USES_HOSTS to 0"
#    else
#      error "Using hosts file for DNS resolution isn't supported on this OS, set RV_DNS_USES_HOSTS to 0 in your configuration file"
#    endif
...
#  else
#    error "RV_DNS_USES_HOSTS requires RV_DNS_USES_CACHING"
#  endif
...
#define EHD_LOG_EXCEP_0(f)                RvLogExcep(self->pLogSource, (self->pLogSource, FUNCTION_NAME ": " f))
#define EHD_LOG_EXCEP_1(f, p1)            RvLogExcep(self->pLogSource, (self->pLogSource, FUNCTION_NAME ": " f, p1))
#define EHD_LOG_EXCEP_2(f, p1, p2)        RvLogExcep(self->pLogSource, (self->pLogSource, FUNCTION_NAME ": " f, p1, p2))
#define EHD_LOG_EXCEP_3(f, p1, p2, p3)    RvLogExcep(self->pLogSource, (self->pLogSource, FUNCTION_NAME ": " f, p1, p2, p3))
...
static RvUint32 hash(RvChar *key) {
  ...
  while(*key) {
    ...
    result ^= (RvUint32) tolower((RvInt)*key);
    ++key;
  }
...
RvBool RvEHMapFind(RvEHMap *self, RvChar *key, RvEHBaseRecord ***prec) {
    ...
    h = hash(key) & (RvUint32)self->sizeMask;
...
RvStatus RvEHMapReadLine(RvEHMap *self, RvFILE *fp, RvChar **pline, RvSize_t *psize) {
    ...
    for(;;) {
        ...
        lastpos = tmpSize - 2;
        line[lastpos] = 0;
        p = RvFgets(line + size - 1, (RvInt)size + 1, fp);
        size = tmpSize;
        if(p == 0) {
            break;
        }
...
RvStatus RvGetLastModificationTime(const RvChar *fileName, RvUint32 *lmt) {
    struct stat st;
    int r;
...
#define FUNCTION_NAME "RvEHDFind"
    ...
    if((qtype == T_A && arec->nIpv4 > 0) || (qtype == T_AAAA && arec->nIpv6 > 0)) {
        *pArec = arec;
        return RV_OK;
    } 
...
void RvEHDRelease(RvEHD *self) {
#define FUNCTION_NAME "RvEhdRelease"

    if(self->lockCnt > 0) {
        self->lockCnt--;
    } else {
        EHD_LOG_EXCEP_1("Trying to release unlocked instance: %p", self);
    }
