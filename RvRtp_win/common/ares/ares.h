...
#define RV_NS_MAXCDNAME	255	    /* maximum compressed domain name */
...
#include <resolvLib.h> /* removed in Workbench 3.2 */
#else
...
#define NS_MAXDNAME	    1025    /* maximum domain name */ 
    ...
    (RV_OS_TYPE == RV_OS_TYPE_MAC)   || (RV_OS_TYPE == RV_OS_TYPE_IPHONE)     || RV_ANDROID || \
	((RV_OS_TYPE == RV_OS_TYPE_VXWORKS) && (RV_OS_VERSION >= RV_OS_VXWORKS_3_2))
...
#define MAXCDNAME		RV_NS_MAXCDNAME
...
#define	DEFAULT_HEARTBEAT_TIMEOUT		RvInt64Mul(RvInt64Const(1,0,5), RV_TIME64_NSECPERSEC)
...
#define ARES_ENGINE_COUNTER_MASK 28
...
#ifndef RV_DNS_AUTOCONFIG
#  define RV_DNS_AUTOCONFIG (RV_DNS_SERVERS | RV_DNS_SUFFIXES)
#endif
...
struct RvDnsEngineStruct {
	...
	/* chp = Current Highest Priority | priority management section for DNS skipping */
	RvUint8          chpValue;
	RvInt	         chpServer;
	RvUint8          minPriority;
    RvInt64			 heartbeatTimeout;
	RvChar			*strHeartbeatQuery;
	RvTimer			 heartbeatTimer;
	RvInt            heartbeatServer;
	RvDnsQueryType   heartbeatType;
	RvChar          *heartbeatBuffer;
	void            *hHeartbeatPage;
	RvBool           bHbWasProcessed;
	RvBool           bEnableDnsPriority;
};


struct rv_ares_options {
    int flags;
    RvInt64 timeout;
    int tries;
    int ndots;
...
int rv_ares_query(RvDnsEngine *channel, const char *name, int dnsclass, int type, void *query_buf,
               RvSize_t *qbuf_len, RvDnsNewRecordCB newRecordCB, rvAresCallback callback, void *usr_arg, unsigned int qid);

int rv_ares_query_ext(RvDnsEngine *channel, const char *name, int dnsclass, int type, void *query_buf,
               RvSize_t *qbuf_len, RvDnsNewRecordCB newRecordCB, rvAresCallback callback, void *usr_arg, unsigned int qid,
			   RvBool isHeartbeat, int server);
int rv_ares_search(RvDnsEngine *channel, const char *name, int dnsclass,
                 int type, rvAresCallback callback, void *arg, unsigned int qid);

/* Cancels query given by qid */
int rv_ares_cancel_query(RvDnsEngine *channel, unsigned int qid, RvBool waitForCallbacks);
...
int rv_ares_expand_string(const unsigned char *encoded, char *s, int strLen);
