/* $Id: rv_ares_private.h,v 1.3 1998/09/22 01:46:11 ghudson Exp $ */
...
#define RV_DNS_MAX_SENDBUF_SIZE (2 + HFIXEDSZ + RV_NS_MAXCDNAME + QFIXEDSZ)
...
struct query_struct {
    ...
    /* Arguments passed to rv_ares_send() */
    char *qbuf;
    int qlen;
    ...
    RvBool mark;
#endif
	RvBool  isHeartbeat; /* specifies if this query is a heartbeat or not */
    /* Dynamic fields - should be the last field in the structure 
    The in-memory layout of dynamic fields:
    int skip_server[dnsEngine->skipServerWords];
    RvUint16 qlen;
    RvUint8 q[qlen];
    */
    int skip_server[1];  
