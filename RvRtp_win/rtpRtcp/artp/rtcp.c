...
void WriteLogV(const char* szFlag, int nFlag, const char* szLogFmt, va_list vArgList)
{
	FILE * pFile = NULL;
	char szDir[256] = {0};	

	//开启文件
	pFile = fopen(szFlag,  "a");
	if (pFile == NULL)
		return;
...
void WriteLog(const char* szFlag, const char* szLogFmt, ...)
{
	...
	WriteLogV(szFlag, 0, szLogFmt, argptr);
	va_end(argptr);
}
 ...
 * - RvRtcpInit() binds to the 揳ny?IP address.
 ***********************************************************************************/
                ...
                IN  RvRtcpSession  hRTCP)
{
    ...
    if (hRTCP == NULL)
        return RV_ERROR_UNKNOWN;
	RTPLOG_DEBUG((RTP_SOURCE, "RvRtcpClose ---RvRtpAddressListDestruct"));

	

	RvRtcpRemoveAllRemoteAddresses(hRTCP); //update by kongfd ?????
	//RvLockGet(&s->lock, logMgr); // delete ???
	RvRtpAddressListDestruct(&s->addressList);
	//RvLockRelease(&s->lock, logMgr);//delete ????

    /* Cancel the timer if one is active */	
	RTPLOG_DEBUG((RTP_SOURCE, "Cancel the timer if one is active"));
    if (s->isTimerSet)
        RvTimerCancel(&s->timer, RV_TIMER_CANCEL_WAIT_FOR_CB);
#ifdef __H323_NAT_FW__    
    if (NULL != s->demux)   
    {
        ...
        {            
			...
			//RvLogDebug(rvLogPtr, (rvLogPtr, "RvRtcpClose(%#x): closing the demux socke",d));
	...
	RTPLOG_DEBUG((RTP_SOURCE, "RvRtcpClose ---RvLockDestruct"));
                                     ...
                                     IN  RvNetAddress* pRtcpAddress)    /* target ip address and target UDP port */
{
    ...
    else
    {
        ...
        if (pRvAddress != NULL)
        {
			...
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtcpSetRemoteAddress"));
    ...
    IN  RvRtcpSession  hRTCP)
{
	...
	RTPLOG_DEBUG((RTP_SOURCE, "RvRtcpRemoveAllRemoteAddresses begin:(%#x)", s));
	RvLockGet(&s->lock, logMgr);
    ...
    {  
		// add log by kongfd ????
		{
			...
			RvAddressIpv4 * ip4 = RvAddressGetIpv4(&(pNatAddress->address));
			if(ip4 != NULL)
			{
				RvChar buff_temp[64];
				RvChar * buff_temp1=NULL;
				buff_temp1=RvAddressIpv4ToString(buff_temp,sizeof(buff_temp),ip4->ip);
				RTPLOG_DEBUG((RTP_SOURCE, "RvRtcpRemoveAllRemoteAddresses: count:(%#d) IP:(%#s) PORT(%#d) ",count, buff_temp1, ip4->port));

			}
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtcpRemoveAllRemoteAddresses itemPtr:(%#x) isMultiplexed(%#d) multiplexID(%#d) ",pNatAddress, pNatAddress->isMultiplexed, pNatAddress->multiplexID));
		...
		if (count > 1024)
		{
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtcpRemoveAllRemoteAddresses error count:(%#d) session:(%#x) addrlist:(%#x) size:(%#d)", count ,s, &s->addressList, RvObjListSize(&s->addressList)));
			RvObjListReset(&(s->addressList));
			break;
		}

		if(RvObjListSize(&(s->addressList))== 0)//update by kongfd ????
		{
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtcpRemoveAllRemoteAddresses RvObjListSize== 0 session:(%#x) addrlist:(%#x) size:(%#d)", s, &(s->addressList), RvObjListSize(&(s->addressList))));
			RvObjListReset(&(s->addressList));
			break;
		}
...
**/
        ...
        if (pNatAddress->isMultiplexed)
		{
			...
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtcpRemoveAllRemoteAddresses session:(%#x) addrlist:(%#x)", s,&s->addressList));
		}
        else
#endif
		{
            RvRtpAddressListRemoveAddress(&s->addressList, &pNatAddress->address, NULL);
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtcpRemoveAllRemoteAddresses2 session:(%#x) addrlist:(%#x)", s,&s->addressList));
		}
    }
    s->remoteAddressSet = RV_FALSE;
	...
	RTPLOG_DEBUG((RTP_SOURCE, "RvRtcpRemoveAllRemoteAddresses end:(%#x)", s));
		...
		IN		RvAddress*	  remoteAddress)
{
    ...
    while (currPtr < compoundEnd)
    {
		...
		//WriteLog("D:\\log\\rv32rtp.txt", "rtcpProcessCompoundRTCPPacket:%d", hdr_type);
		...
		IN	RvAddress	*remoteAddress)
{
	...
	//WriteLog("D:\\log\\rv32rtp.txt", "rtcpProcessRTCPPacket:%d", type);
    switch(type)
    {
        case RTCP_SR:
        case RTCP_RR:
    ...
    IN RvUint32*      pMultiplexerId)
{
    ...
    if (pRvAddress != NULL)
    {
		...
		//RvLockRelease(&s->lock, logMgr);
		RTPLOG_DEBUG((RTP_SOURCE, "rtcpDemuxAddRemoteAddress session:(%#x) addrlist:(%#x)", s, &s->addressList));
        s->remoteAddressSet = RV_TRUE;
    ...
    IN  RvUint32*      pMultiplexID)
{
    ...
    else
	{
		...
		if (s!=NULL && 	s->profilePlugin->funcs->removeRemAddress != NULL)
			...
			RTPLOG_DEBUG((RTP_SOURCE, "rtcpDemuxRemoveRemoteAddress"));
...
RvStatus rtcpSetRtpSession(IN RvRtcpSession hRTCP, IN RvRtpSession hRTP)
{
	...
	RvLogDebug(rvLogPtr, (rvLogPtr, "rtcpSetRtpSession: RTCP=0x%#x, RTP=0x%#x", hRTCP, hRTP));
	RvLogLeave(rvLogPtr, (rvLogPtr, "rtcpSetRtpSession"));
    return RV_OK;
}
...
static RvInt32 update_seq(rtpSource *s, RvUint16 seq, RvUint32 ts, RvUint32 arrival)
{
    ...
    else if (udelta < MAX_DROPOUT)
    {
        ...
        //s->max_seq = seq;
		//zzx
		if (seq>s->max_seq && seq-s->max_seq<MAX_DROPOUT)
		{
			s->max_seq = seq;
		}
		else if (seq<s->max_seq && s->max_seq-seq>MAX_DROPOUT)
		{
    ...
    for (i=0; i < initSessionMembers; i++)
    {
		...
		if (pInfo->tLastReport > 0)
		{
			...
			if (timeSinceLastReport > (5 * s->txInterval.baseTimeInterval))
			{
				...
				if (res == RV_TRUE)
				{
                    //deleted by lichao, 20150729 由外部调用(xt_mp_xxx库)来主动删除远端rtcp地址
