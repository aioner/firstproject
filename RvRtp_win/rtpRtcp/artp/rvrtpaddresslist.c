	...
	if(succeded == RV_TRUE) {
		...
		if(itemPtr != NULL) 
        {
...
#endif
			{
				...
				RvAddressIpv4 * ip4 = RvAddressGetIpv4(&(itemPtr->address));
				if(ip4 != NULL)
				{
					RvChar buff_temp[64];
					RvChar * buff_temp1=NULL;
					buff_temp1=RvAddressIpv4ToString(buff_temp,sizeof(buff_temp),ip4->ip);
					RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListAddAddress: IP:(%#s) PORT(%#d) ",buff_temp1, ip4->port));

				}
			}
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListAddAddress itemPtr:(%#x) elem(%#x) ",itemPtr, itemPtr->elem));
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListAddAddress isMultiplexed(%#d) multiplexID(%#d)", itemPtr->isMultiplexed, itemPtr->multiplexID));
			//RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListAddAddress poolElem-obj(%#x) poolElem-prev(%#x) poolElem-next(%#x)", itemPtr->poolElem.obj, itemPtr->poolElem.prev, itemPtr->poolElem.next));
                                     ...
                                     IN RvUint32* pMultiplexID)
{
	...
	RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListRemoveAddress begin:(%#x)", thisPtr));
    ...
    while(itemPtr != NULL) 
    {
		if( RvObjListSize(thisPtr) == 0)
		{
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListRemoveAddress RvObjListSize ==0 addrlist:(%#x) size:(%#d)",  thisPtr, RvObjListSize(thisPtr)));
			RvObjListReset(thisPtr);
			break;
		}
		...
		if (count > 1024)
		{
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListRemoveAddress error count:(%#d) addrlist:(%#x) size:(%#d)", count, thisPtr, RvObjListSize(thisPtr)));
			break;
		}
        ...
        {
			{
				...
				if(ip4 != NULL)
				{
					...
					RvChar * buff_temp1=NULL;
					buff_temp1=RvAddressIpv4ToString(buff_temp,sizeof(buff_temp),ip4->ip);
					RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListRemoveAddress: count:(%#d) IP:(%#s) PORT(%#d) ",count, buff_temp1, ip4->port));

				}
			}
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListRemoveAddress itemPtr:(%#x) elem(%#x) ",itemPtr, itemPtr->elem));
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListRemoveAddress isMultiplexed(%#d) multiplexID(%#d)", itemPtr->isMultiplexed, itemPtr->multiplexID));
			//RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListRemoveAddress poolElem-obj(%#x) poolElem-prev(%#x) poolElem-next(%#x)", itemPtr->poolElem.obj, itemPtr->poolElem.prev, itemPtr->poolElem.next));
	...
	RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListRemoveAddress end:(%#x)", thisPtr));
...
RvBool RvRtpAddressListRemoveAllAddresses(RvRtpAddressList *thisPtr)
{
	...
	size = RvObjListSize(objlist);
	RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListRemoveAllAddresses size:(%#d)", size));
	...
	{
		{
			...
			if(ip4 != NULL)
			{
				...
				RvChar * buff_temp1=NULL;
				buff_temp1=RvAddressIpv4ToString(buff_temp,sizeof(buff_temp),ip4->ip);
				RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListRemoveAllAddresses: count:(%#d) IP:(%#s) PORT(%#d) ",count, buff_temp1, ip4->port));

			}
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListRemoveAllAddresses itemPtr:(%#x) elem(%#x) ",itemPtr, itemPtr->elem));
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListRemoveAllAddresses isMultiplexed(%#d) multiplexID(%#d)", itemPtr->isMultiplexed, itemPtr->multiplexID));
			//RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListRemoveAllAddresses poolElem-obj(%#x) poolElem-prev(%#x) poolElem-next(%#x)", itemPtr->poolElem.obj, itemPtr->poolElem.prev, itemPtr->poolElem.next));
		}
		
		if( RvObjListSize(objlist) == 0)
		{ 
			RvObjListReset(objlist);
			break;
		}
		if(count > size)
		{
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListFreeItem count:(%#d) obj:(%#x) itm:(%#x) count >size?????????", count, objlist, itemPtr));
			break;
		}
		if(count > 1024)
		{
			RTPLOG_DEBUG((RTP_SOURCE, "RvRtpAddressListFreeItem count:(%#d) obj:(%#x) itm:(%#x) count > 1024?????????", count, objlist, itemPtr));
			break;
		}
