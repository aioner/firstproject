        ...
        IN  RvRtpSession  hRTP)
{
    ...
    if (s)
    {
        ...
        if (NULL != s->demux)
        {
            ...
            if (d->rtpSessionsCounter <= 0)
            {
                ...
                d->rtpTransport = NULL;
				RTPLOG_DEBUG((rvLogPtr, "RvRtpClose(%#x): closing the demux socket2", d));
            }
...
#endif
        {
			...
			RTPLOG_DEBUG((rvLogPtr, "RvRtpClose(%#x): closing the socket2", s));
        }
	...
	RTPLOG_DEBUG((rvLogPtr, "RvRtpClose(): end"));
    return RV_OK;
}
...
RvStatus RVCALLCONV RvRtpRemoveAllRemoteAddresses(IN RvRtpSession  hRTP)
{
	...
	{
		RTPLOG_DEBUG((rvLogPtr, "RvRtpRemoveAllRemoteAddresses begin"));
		RvLockGet(&s->lock, logMgr);
		...
		{	
			// add log by kongfd ????
			{
				...
				if(ip4 != NULL)
				{
					...
					RTPLOG_DEBUG((rvLogPtr, "RvRtpRemoveAllRemoteAddresses: count:(%#d) IP:(%#s) PORT(%#d) ",count, buff_temp1, ip4->port));

				}
				RTPLOG_DEBUG((rvLogPtr, "RvRtpRemoveAllRemoteAddresses itemPtr:(%#x) isMultiplexed(%#d) multiplexID(%#d) ",pNatAddress, pNatAddress->isMultiplexed, pNatAddress->multiplexID));
			}
			//add end
			...
			if (count > 1024)
			{
				RTPLOG_DEBUG((rvLogPtr, "RvRtpRemoveAllRemoteAddresses error count:(%#d) session:(%#x) addrlist:(%#x) size:(%#d)", count ,s, &(s->addressList), RvObjListSize(&(s->addressList))));
				RvObjListReset(&(s->addressList));
				break;
			}
			if(RvObjListSize(&(s->addressList))== 0)//update by kongfd ????
			{
				RTPLOG_DEBUG((rvLogPtr, "RvRtpRemoveAllRemoteAddresses RvObjListSize== 0 session:(%#x) addrlist:(%#x) size:(%#d)", s, &(s->addressList), RvObjListSize(&(s->addressList))));
				RvObjListReset(&(s->addressList));
				break;
			}
...
**/
		...
		RTPLOG_DEBUG((rvLogPtr, "RvRtpRemoveAllRemoteAddresses end"));
	}
