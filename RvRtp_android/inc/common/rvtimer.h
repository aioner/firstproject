...
typedef struct {
    ...
    RvInt64 prevCurtime;        /* the saved during the previous TimerQueueService timestamp */
...
/* Tests whether the timer is active or has been cancelled by the application.
   Please note that this macro is not thread-safe. That is if the one thread cancels
   the timer and the other thread simultaneously tests whether it was cancelled 
   the result is unpredicted.
   Another point is that when the timer fires the application owned RvTimer object 
   is not modified. This is because the timer engine has no access to the application
   owned RvTimer objects.
 */
#define RvTimerIsActive(_t) ((_t)->id != 0)
