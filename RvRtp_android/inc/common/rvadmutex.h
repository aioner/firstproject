...
typedef struct _RvMutex {
	RvAdMutex   mtx;
	RvInt32    count;
#if defined(RV_MUTEX_DEBUG)
    const RvChar    *filename;
    RvInt           lineno;
    struct _RvMutex *prev;
    struct _RvMutex *next;
    RvThreadId owner;
#endif
} RvMutex;
...
#if !defined(RV_MUTEX_HAS_TRYLOCK)
#define RV_MUTEX_HAS_TRYLOCK 0
#endif

#if RV_MUTEX_HAS_TRYLOCK
RvStatus RvAdMutexTryLock(RvMutex *mu, struct RvLogMgrInternal *logMgr);
#else
#define RvAdMutexTryLock(mu, logMgr) RvAdMutexLock(mu,logMgr)
#endif
#ifndef RV_USE_MACROS
/********************************************************************************************
 * RvAdMutexUnlock
