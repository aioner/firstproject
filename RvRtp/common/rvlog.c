    ...
    IN RvLogMgr* logMgr)
{
    ...
    logMgr->level = RV_LOG_MASK; /* Default is log by the masks */
    ...
    IN void*        userData)
{
    ...
    if (ret == RV_OK)
    {
        if (logMgr->numListeners < RV_LOG_MAX_LISTENERS)
        {
            ...
            /* Add the listener , only if not already registered */
            if (ret == RV_OK)
            {
...
/********************************************************************************************
 * RvLogFindListenerCtxByCb - Find a listener set to the log.
 */
RVCOREAPI
void* RVCALLCONV RvLogFindListenerCtxByCb(
    IN RvLogMgr*    logMgr,
    IN RvLogPrintCb listener)
{
    RvStatus ret;
    ...
    if (ret == RV_OK)
    {
        ...
        for (i = 0; i < logMgr->numListeners; i++)
        {
            if (logMgr->listener[i] == listener)
            {
                ...
                return logMgr->listenerUserData[i];
            }
        }
#endif  /* (RV_CHECK_MASK & RV_CHECK_OTHER) */
        RvLockRelease(&logMgr->lock,NULL);
...
#if (RV_CHECK_MASK & RV_CHECK_RANGE)
    if (level < RV_LOG_NONE  ||  level > RV_LOG_ALL)
        return RvLogErrorCode(RV_ERROR_BADPARAM);
#endif
    ...
    IN va_list*         varArg)
{
    ...
    /* Thread Checker: there is no race condition on read/write access of 
                       global variables rvLogInsideTextAny and rvLogCurId,
                       because thread, which is writing into them now, doesn't
                       read from them. All other threads will pass this check
                       and wait for critical section on rvLogLock. */
