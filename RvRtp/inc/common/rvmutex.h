...
#ifdef RV_USE_MACROS
#define RvMutexTryLock(mu, logMgr) RvAdMutexTryLock(mu, logMgr)
#else
#  ifndef RV_MUTEX_DEBUG
    RVCOREAPI 
    RvStatus RVCALLCONV RvMutexTryLock(RvMutex *mu, RvLogMgr *logMgr);
#  else
#   define RvMutexTryLock(mu, logMgr) RvMutexTryLockDbg(mu, logMgr, __FILE__, __LINE__)
    RVCOREAPI
